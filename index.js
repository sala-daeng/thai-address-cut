...
function getValueByKey(object, key) {
    ...
    if(key != null && key != undefined && object != null){
        ...
        return object[Object.keys(object)
          .find(k => k.toLowerCase().replace('khet','').replace(regex,'').trim() === asLowercase)
        ];
    }
    return undefined
}
...
function getKeyByValue(object, value) {
    ...
    return Object.keys(object).find(key => 
        regex.test(key) && object[key] === value
        );
  }
...
module.exports = {
    cutAddress: (address, fullSearch = false) => {
        ...
        for(var i = 0 ; i < 2 && count <=2 ; i++){
            cityValue = getValueByKey(city[provinceValue], wordlist[wordlist.length - 1])
            if (cityValue) {
                ...
                cityTxt = (findSimilarObj(city[provinceValue], wordlist[wordlist.length - 1]))
                cityTempTxt = wordlist[wordlist.length - 1]
                cityValue = getValueByKey(city[provinceValue], cityTxt)
            }
    
            if(wordlist.length - 2 > 0){
                tambonValue = getValueByKey(tambon[cityValue], wordlist[wordlist.length - 2])
                if (tambonValue) {
                    ...
                    tambonTxt = (findSimilarObj(tambon[cityValue], wordlist[wordlist.length - 2]))
